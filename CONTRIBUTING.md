...
1. Make sure the CLI works on your machine: `uv run vibekit --help`
...
When working on vibekit:

1. Test changes with the `vibekit` CLI commands (`/vibekit.specify`, `/vibekit.plan`, `/vibekit.tasks`) in your coding agent of choice
...
Running `uv run vibekit init` pulls released packages, which won’t include your local changes.  
